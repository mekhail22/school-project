...
if st.session_state.page == "home":
    ...
    # ------------------ حقل البحث Uiverse.io (HTML + JS يرسل قيمة للـ Streamlit) ------------------
    st.markdown("""
    <div class="search-container" style="display:flex; justify-content:center;">
      <div class="searchBox">
        ...
        <button class="searchButton" id="searchBtn">بحث</button>
      </div>
    </div>

    <script>
    // عند الضغط على Enter داخل الحقل أو على الزر -> نرسِل رسالة للأب (streamlit)
    ...
    if (inputEl) {
      inputEl.addEventListener('keydown', function(e){
        if (e.key === 'Enter') {
          ...
          window.parent.postMessage({type: 'searchStudent', value: v}, '*');
        }
      });
    }
    if (btnEl) {
    ...
    # هذا الحقل Streamlit له placeholder فريد يساعد الجافاسكربت يملّيه reliably
    hidden_placeholder = "___student_hidden_input_unique_placeholder_12345___"
    student_name = st.text_input("", placeholder=hidden_placeholder, key="student_search_input", label_visibility="collapsed")

    # JS داخل الصفحة يلتقط الرسالة ويملأ حقل Streamlit بناءً على الـ placeholder
    st.markdown(f"""
    <script>
    window.addEventListener('message', (event) => {{
        if (event.data?.type === 'searchStudent') {{
            const value = event.data.value || "";
            const streamlitInputs = window.parent.document.querySelectorAll('input[placeholder="{hidden_placeholder}"]');
            if (streamlitInputs && streamlitInputs.length > 0) {{
                const el = streamlitInputs[0];
                el.value = value;
    ...
    # ------------------ معالجة وإظهار النتائج ------------------
    if student_name and student_name.strip():
        df_student = get_student_records(student_name.strip())
        if df_student.empty:
            st.info("لا يوجد غياب مسجل لهذا الاسم.")
        else:
            st.dataframe(df_student.reset_index(drop=True), use_container_width=True)
            pdf_buf = generate_student_pdf(student_name.strip(), df_student)
            st.download_button("تحميل PDF", data=pdf_buf, file_name=f"{student_name.strip()}_report.pdf", mime="application/pdf")
    else:
        st.info("اكتب اسمك الثلاثي في حقل البحث ثم اضغط (بحث) أو Enter.")
